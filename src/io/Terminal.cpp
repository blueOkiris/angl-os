...
inline void updateCursor(const size_t &x, const size_t &y) {
...
inline void shiftScreenUp(const TerminalColor &color) {
    for(size_t y = 1; y < VGA_HEIGHT - 1; y++) {
        for(size_t x = 0; x < VGA_WIDTH; x++) {
            ...
            const uint16_t currVal = *((uint16_t *) (VGA_MEMORY + currInd * 2));
            *((uint16_t *) (VGA_MEMORY + prevInd * 2)) = makeVgaEntry(
                currVal, color
            );
        }
    }
    for(size_t x = 0; x < VGA_WIDTH; x++) {
        ...
        *((uint16_t *) (VGA_MEMORY + index * 2)) = makeVgaEntry(' ', color);
    }
}
...
void Terminal::putChar(const char &c) {
    if(c == '\n') {
        ...
        putEntryAt(c, _color, _col, _row);
    }
    if(++_col == VGA_WIDTH) {
        _col = 0;
        if(++_row == VGA_HEIGHT) {
            _row = VGA_HEIGHT - 1;
            shiftScreenUp(_color);
        }
    }
    updateCursor(_col, _row);
}
